...
from src.data_processing_real import processingDataReal
...
data_directory = './Realdata/medmal_pos/'
model_directory = './runs/model/'

table_files = os.listdir(data_directory)
table_files = sorted(table_files, key=lambda x: int(x.split('.')[0]))
...
for table_file in table_files:
    table_num = int(table_file.split('.')[0])
    print(f"Processing, table {table_num}")
    
    # 加载和处理数据
    (LossMin, LossMax, trainValid_input, trainValid_output, test_input, test_output) = processingDataReal(
        data_directory, table_num, HYPERPARAMETERS['window_size']
    )
...
results_df.to_csv('Best_Performance.csv', index=False)
print("All tables completed, best performance metrics are saved in 'Best_Performance.csv'.")
