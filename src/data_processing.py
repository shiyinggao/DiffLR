...
def processingData4(data_directory, EnvironmentNumber, TableNumber, window_size, Stage_Size=40, Val_Size=4, Distribution='normal'):
    """
    Data preprocessing function use orginal data in train set
    """
    
    data_file = data_directory + 'Environment ' + str(EnvironmentNumber) + '/Table ' + str(TableNumber) + '.csv'
    data = pd.read_csv(data_file, header=0, index_col=0).to_numpy()
